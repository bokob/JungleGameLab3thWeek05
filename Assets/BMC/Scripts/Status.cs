...
public class Status : MonoBehaviour
{
    ...
    {
        _isDead = true;                             // 죽음 상태
        Manager.Game.SpawnedList.Remove(transform); // 소환된 리스트에서 제거
        _collider.enabled = false;                  // Collider 비활성화
        _anim.SetTrigger("DieTrigger");             // 죽는 애니메이션 Trigger
        DieAction?.Invoke();                        // 각각의 죽는 행동 실행
        Destroy(gameObject, 10f);                   // 10초 후에 삭제
    }
}