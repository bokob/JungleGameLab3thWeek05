...
public class TaskRetreatAndAttack_Archer : Node
{
    ...
    public override NodeState Evaluate()
    {
        ...
        if (distance < ArcherBT.MinRange) // 최소 거리보다 가까우면 뒤로 이동
        {
           ...
           _enemy.Flip(_transform, -dir.x); // 방향 반전 (적이 뒤에 있으므로)
            ...
            Vector3 retreatPosition = _transform.position + (Vector3)dir * (ArcherBT.AttackRange - distance);
            _transform.position = Vector3.MoveTowards(
                _transform.position,
                retreatPosition,
                ArcherBT.MoveSpeed * Time.deltaTime);
        }
