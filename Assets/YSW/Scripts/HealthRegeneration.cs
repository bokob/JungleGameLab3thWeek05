...
{
    private int _necromancerKillCount = 0; // 네크로맨서 처치 카운트
    private Coroutine _regenerationCoroutine; // 현재 실행 중인 재생 코루틴
    private Status _playerStatus; // 플레이어의 Status 참조
    private float _lastHP; // 마지막으로 확인한 체력 값
    private bool _isRegenerationActive = false; // 재생 효과가 활성화 중인지 여부
    [SerializeField] private ParticleSystem _healingParticle; // 회복 파티클 (private 선언)
